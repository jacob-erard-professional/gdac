...
def compute_brand_frequency(input_file: Path, *, chunk_size: int = 100_000) -> Counter:
    ...
    for chunk in pd.read_csv(input_file, dtype=str, keep_default_na=False, chunksize=chunk_size):
        ...
        for value in chunk[column].tolist():
            for brand in _parse_brands(value):
                counter[brand.lower()] += 1
    return counter


def main() -> None:
    parser = argparse.ArgumentParser(description="Compute brand frequency from raw tweets CSV.")
    ...
    if output_file is None:
        ...
        output_file = base_dir / "outputs" / "analytics" / year / "brand_name_frequency.json"

    counter = compute_brand_frequency(input_file, chunk_size=args.chunk_size)
    rows = [{"brand": brand, "count": count} for brand, count in counter.most_common()]

    output_file.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "year": year,
        "brands": rows,
        "total_brands": int(sum(counter.values())),
        "unique_brands": int(len(counter)),
    }
