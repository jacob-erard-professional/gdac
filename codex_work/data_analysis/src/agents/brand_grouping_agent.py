...
def _map_chunk_to_brands(invoker: _RateLimitedInvoker, chunk: List[HashtagCount]) -> Dict[str, str]:
    ...
    prompt = (
        "Group hashtags by real-world brand/property.\n"
        "Output strict JSON only with schema:\n"
        '{"mappings":[{"hashtag":"<string>","brand":"<canonical brand or unassigned>"}]}\n'
        "Rules:\n"
        "- brand values are lowercase short names like dc, pepsi, nfl, unassigned.\n"
        "- if two hashtags refer to the same brand, use exactly the same brand label.\n"
    ...
    for mapping in mappings:
        ...
        brand = str(mapping.get("brand", "")).strip().lower() or "unassigned"
        if hashtag:
            out[hashtag] = brand
    for item in chunk:
        out.setdefault(item.hashtag, "unassigned")
    return out
...
def run_brand_grouping(
    ...
    for item in hashtags:
        label = hashtag_to_brand.get(item.hashtag, "unassigned")
