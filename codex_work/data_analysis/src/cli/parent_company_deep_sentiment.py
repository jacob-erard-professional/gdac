...
def parent_company_deep_sentiment_command(
    year: str = typer.Option(None, "--year"),
    data_dir: Path = typer.Option(None, "--data-dir"),
    deep_sentiment_file: Path = typer.Option(None, "--deep-sentiment-file"),
    ...
    resolved_deep_sentiment = deep_sentiment_file or (base_dir / "sentiment" / "deep" / cfg.year / "deep_sentiment.json")
    ...
    outputs, manifest = run_parent_company_deep_sentiment_analysis(
        year=int(cfg.year),
        deep_sentiment_path=resolved_deep_sentiment,
    ...
    typer.echo(f"parent company deep sentiment joined output written to {outputs.joined_parquet}")
    typer.echo(f"parent company deep sentiment summary written to {outputs.summary_json}")
