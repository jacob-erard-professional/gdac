...
def test_full_year_with_parent_company_sentiment_runs_stage(monkeypatch, tmp_path: Path):
    ...
    captured = {"sentiment": False, "parent_company_sentiment": False}
    ...
    def fake_parent_company_sentiment_run(config, **kwargs):
        captured["parent_company_sentiment"] = True
        return type("Manifest", (), {
    ...
    monkeypatch.setitem(orchestrator.RUNNERS, "parent_company_sentiment", fake_parent_company_sentiment_run)
    ...
    req = RunRequest(
        ...
        data_dir=None,
        with_sentiment=True,
        with_parent_company_sentiment=True,
        sentiment_dry_run=True,
    )
    result = run_pipeline(tmp_path, req)[0]

    assert captured["sentiment"] is True
    assert captured["parent_company_sentiment"] is True
    stage_names = [s["stage"] for s in result.stages]
    assert "sentiment" in stage_names
    assert "parent_company_sentiment" in stage_names
